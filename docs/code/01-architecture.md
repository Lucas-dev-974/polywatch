...
| **weather-algo** | Signaux météo → enqueue `weather-order-signals` ; closes drift/pre-close → `close-signals` |
...
- **weather-algo → worker** : file Redis `weather-order-signals` (`WEATHER_OPEN`) et `close-signals` (`WEATHER_FORECAST_CHANGE`, `WEATHER_PRE_CLOSE`).
