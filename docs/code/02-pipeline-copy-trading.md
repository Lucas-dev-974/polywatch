...
2. **Filtre type de marché** (`passesMarketTagFilter`) : si `simAllowedMarketTags` / `realAllowedMarketTags` est non vide, `MarketService.resolveTagSlugs(conditionId)` puis `isMarketTagAllowedForMode` (`core/src/risk/policy.ts`). Skip si aucun slug en commun. Les sorties ne passent pas par ce filtre.
3. **Filtre minTimeToClose** (`copy-entry-pipeline.ts` — `getModeMinTimeToClose(risk, mode)`) : si le temps restant avant la clôture du marché (`timeToEndMs`) est inférieur ou égal à `minTimeToClose * 1000`, l'entrée est refusée. Empêche les copies trop proches de la résolution du marché.
...
7. **Filtre bid/ask** : `isEntryBidAskRatioAcceptable(bidVwap, askVwap, getModeMinBidToAskRatio(risk, mode))` — skip si le bid exécutable est trop bas par rapport à l'ask (spread extrême). Seuil par mode dans `risk_config` (`*MinBidToAskRatio`, défaut `0.9`, `0` = off).
