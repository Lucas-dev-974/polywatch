...
Utiliser l'endpoint `/api/wallet/approvals` pour vérifier et approuver.
...
| Approvals | `GET /api/wallet/approvals` (tous à `true`) |
| WebSocket user | Logs : `WebSocket user channel connected` |
| Mode réel | `SELECT real_trading_enabled FROM risk_config;` (`false` par défaut) |
...
```sql
UPDATE risk_config SET real_trading_enabled = true;
```
...
```sql
UPDATE risk_config SET real_entry_usdc_amount = 5;  -- $5 USD
```
...
psql "$DATABASE_URL" -c "UPDATE risk_config SET real_trading_enabled = false;"
...
- Pipeline : [`docs/pipeline-copy-trading.md`](./pipeline-copy-trading.md)
- API : [`docs/api.md`](./api.md)
- Métriques : [`docs/metrics.md`](./metrics.md)