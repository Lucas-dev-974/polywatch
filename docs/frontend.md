...
| **Weather Algo** | `WeatherAlgoPage` : onglets **Marchés** (`WeatherAlgoActiveMarketsPanel` + `WeatherAlgoDiscoverPanel` + `WeatherAlgoForecastPanel`), **Positions** (`WeatherAlgoPositionsPanel` + `WeatherAlgoExecutionsPanel`), **Auto-track** (`WeatherAlgoAutoTrackTab`), **Paramètres** (`WeatherAlgoSettingsTab`). Les marchés découverts et suivis sont groupés par ville dans des accordions (`WeatherCityGroup`). Header : `WeatherAlgoHeader` (statut, heartbeat). Hooks : `useWeatherAlgoDashboard`, `useWeatherAlgoPositions` |
