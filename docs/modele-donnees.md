...
| `WeatherClobPriceHistory` | `weather_clob_price_history` | Historique prix CLOB Polymarket par bucket météo (ingestion manuelle) — index unique `(condition_id, side, recorded_at)` ; colonnes `city`, `target_date`, `metric`, `bucket_*`, `token_id`, `price`, `fidelity_minutes`, `ingest_job_id`. Pour un marché résolu, un **point de settlement synthétique** (1.00 gagnant / 0.00 perdant) est ajouté en fin de série (voir `weather-algo.md` § Ingestion historique CLOB) |
