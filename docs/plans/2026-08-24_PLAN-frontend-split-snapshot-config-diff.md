...
├── diff.ts           ← logique de comparaison de configs (le cœur du diff)
├── legacy-mapping.ts ← mapping des anciens formats/champs legacy
└── index.ts          ← baril ré-exportant l'API publique
```

> **Note** : `lib/snapshot-config-display.ts` (58 l) **existe déjà** comme module séparé avec son test (`snapshot-config-display.test.ts`). Il gère déjà le formatage/labels. Ne pas le recréer dans ce dossier — il reste à sa place actuelle. Ce plan ne scinde que `diff` + `legacy-mapping` depuis `snapshot-config-diff.ts`.
...
2. **Déplacer** les fonctions dans les 2 modules (`diff` / `legacy-mapping`) en fonction de leur responsabilité. Le formatage/labels reste dans `lib/snapshot-config-display.ts` déjà existant — ne pas y toucher.
3. **Créer `index.ts`** qui re-exporte tout — les imports existants ne changent PAS (pattern baril déjà utilisé pour `api.ts`).
4. **Adapter le test** `snapshot-config-diff.test.ts` si ses imports profonds doivent changer, sinon le laisser.
