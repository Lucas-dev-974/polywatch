...
| Entrée | Comportement |
|--------|----------------|
| Nav **Rapports** | Hub complet : bibliothèque, éditeur, viewer, comparaison |
...
- Exposé sur `GET /api/risk-config` et chaque rapport enregistré.
- **Apply recommandations** (dialog ou hub) envoie `expectedCryptoAlgoConfigFingerprint`
  dans `PUT /api/risk-config` → **409** `config_fingerprint_mismatch` si la config live
  a changé depuis la génération.

Champs optionnels acceptés par `PUT /api/risk-config` (hors schéma strict) :
...
Append-only à **chaque** `PUT /api/risk-config` : `config_json` (API présentée),
