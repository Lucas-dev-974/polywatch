...
| GET | `/api/market-chart/order-size` | JWT | Taille d'ordre minimale CLOB pour un `assetId` (query) — source `book` ou fallback |
...
### Crypto-Algo Data (`/api/crypto-algo-data/*`)

Persistance/audit des données enregistrées par le crypto-algo (service
`CryptoAlgoDataService`). Toutes les routes requièrent JWT.

| Méthode | Route | Description |
|---------|-------|-------------|
| GET | `/api/crypto-algo-data/tables` | Résumé des tables enregistrées (compteurs, taille) |
| GET | `/api/crypto-algo-data/coverage` | Couverture temporelle des données enregistrées |
| GET | `/api/crypto-algo-data/price-ticks` | Ticks de prix algo (`?conditionId=&from=&to=&limit=&offset=`) |
| GET | `/api/crypto-algo-data/surveillance-snapshots` | Snapshots de surveillance OHLC (`?limit=&offset=`) |
| GET | `/api/crypto-algo-data/post-entry-mid-samples` | Échantillons mid post-entrée (`?conditionId=&from=&to=&limit=&offset=`) |
| GET | `/api/crypto-algo-data/market-selections` | Sélections de marchés (`?enabled=&limit=&offset=`) |
| GET | `/api/crypto-algo-data/auto-track-rules` | Règles d'auto-track (`?enabled=&limit=&offset=`) |
| GET | `/api/crypto-algo-data/executions` | Exécutions enregistrées (`?conditionId=&mode=&status=&from=&to=&limit=&offset=`) |
| GET | `/api/crypto-algo-data/positions` | Positions enregistrées (`?conditionId=&mode=&status=&from=&to=&limit=&offset=`) |
| DELETE | `/api/crypto-algo-data/tables` | Purge toutes les données enregistrées |
| DELETE | `/api/crypto-algo-data/tables/:id` | Purge une table spécifique (`id` ∈ `CRYPTO_ALGO_DATA_TABLE_IDS`) ; `403 read_only_table` si lecture seule |
...
| `winningOutcome` | `"YES"` \| `"NO"` | oui | Outcome gagnant à racheter |
| `quantityRaw` | string | oui | Quantité de parts (raw/entier, ex. `"5120000"`) |
...
| `real_snapshot_created` | positions | Nouveau snapshot trading réel enregistré |
| `real_period_rotated` | positions, executions | Rotation de période réelle |
| `system:audit:started` | (broadcast) | Début d'un audit système |
| `system:audit:log` | (broadcast) | Ligne de log d'audit système |
| `system:audit:finished` | (broadcast) | Fin d'un audit système |
...
| GET | `/api/weather-algo/strategy-catalog` | JWT | Catalogue stratégies (`WEATHER_STRATEGY_CATALOG` : id, label, description, `supportsGroup`, `params[]`) — 3 stratégies : `weather-forecast`, `weather-forecast-aligned`, `weather-highest-yes` |

### Weather Algo data (persistance / audit)

Routes JWT sous `/api/weather-algo-data`. Service : `WeatherAlgoDataService`.  
Doc : [`../weather/plans/2026-08-08_IMPL-weather-market-data-persistence.md`](../weather/plans/2026-08-08_IMPL-weather-market-data-persistence.md).
...
| GET | `/api/backtest/runs/:id/excluded-ticks` | Ticks exclus du run (tracés ridge) `{ ticks: [{ t, reason, city, conditionId, metric }] }` |
