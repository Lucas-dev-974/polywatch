...
Routes montées (source de vérité : `packages/backend/src/index.ts`) :
...
```
  ...
  app.use('/api/traders', …)
  app.use('/api/market-tags', …)
  app.use('/market-icons', …)
  app.use('/api/markets', …)
  app.use('/api/algo-markets', …)
  app.use('/api/algo-auto-track', …)
  app.use('/api/algo/executions', …)
  app.use('/api/algo/capital', …)
  app.use('/api/algo/markets-prices', …)
  app.use('/api/algo/surveillance-history', …)
  app.use('/api/algo/events', …)
  app.use('/api/algo/market-chart', …)
  app.use('/api/market-chart', …)
  app.use('/api/copied-positions', …)
  app.use('/api', …)              // createConfigRouter, createConfigPerKindRouter
  app.use('/api', …)              // createSimulationRouter, createRealSessionsRouter
  app.use('/api/executions', …)
  app.use('/api/move-events', …)
  app.use('/api/wallet', …)
  app.use('/api/e2e-runs', …)
  app.use('/api', …)              // createMarketSyncConfigRouter
  app.use('/api/system-config', …)
  app.use('/api/system', …)      // createSystemOverviewRouter, createSystemAuditRouter
  app.use('/api/system/crypto-algo-monitor', …)
  app.use('/api/crypto-algo-data', …)
  app.use('/api/reports', …)
  app.use('/api/weather-algo-markets', …)
  app.use('/api/weather-algo-discover', …)
  app.use('/api/weather-algo-forecasts', …)
  app.use('/api/weather-algo-auto-track', …)
  app.use('/api/weather-algo/executions', …)
  app.use('/api/weather-algo/capital', …)
  app.use('/api/weather-algo', …) // strategy catalog
  app.use('/api/weather-algo-data', …)
  app.use('/api/weather-algo-history', …)
  app.use('/api/backtest', …)
  app.use('/api/internal', …)    // createInternalRouter
```

> La liste détaillée des endpoints par routeur (méthode, chemin, description)
> figure dans [`api.md`](./api.md). Ce bloc reste volontairement synthétique ;
> consulter `packages/backend/src/index.ts` pour la liste exacte et à jour.
...
Voir [`../code/05a-copy-trading.md`](../code/05a-copy-trading.md).
...
Voir [`../code/07-crypto-algo.md`](../code/07-crypto-algo.md) pour le détail.

### Weather-Algo (`packages/weather-algo/src/index.ts`)

Process sans serveur HTTP — trading météo **par ville**. Détail :
[`weather-algo.md`](./weather-algo.md) · [`../code/08-weather-algo.md`](../code/08-weather-algo.md).
...
- Sorties : `WEATHER_FORECAST_CHANGE` / `WEATHER_BUCKET_EXIT` (`close_and_reenter` + hysteresis, ou `hold`). La sortie `WEATHER_PRE_CLOSE` a été **retirée**.
...
| File Redis `close-signals` | worker Strategy / weather-algo → worker Executor | SL/TP/pre-close/kill-switch / closes manuels / `WEATHER_FORECAST_CHANGE` / `WEATHER_BUCKET_EXIT` |
