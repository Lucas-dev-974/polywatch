...
- **Regle d'entree (bande activee, defaut)** : entree si le **prix du token achete** est strictement dans `(entryPriceMin, entryPriceMax)` — defaut **(0,55 ; 0,80)** :
  - **YES** si `0,55 < prix Up < 0,80`
  - **NO** si `0,55 < prix Down < 0,80` (equivalent : `0,20 < prix Up < 0,45`)
...
| Historique WS insuffisant | fail-closed (`curve_insufficient`) |
...
Tests : `npm run test:e2e:crypto:sim-reset` · Plan : [`plans/applied/2026-07-12_PLAN_SIM_RESET_REDIS_HYGIENE.md`](../plans/applied/2026-07-12_PLAN_SIM_RESET_REDIS_HYGIENE.md)
...
| `cryptoAlgoWarnPriceDeviation` | 0.05 | Log ecart WS/Gamma (non-bloquant) |
| `cryptoAlgoAlertPriceDeviation` | 0.15 | Seuil ecart WS/Gamma declenchant une alerte operateur (banner UI via `POST /api/internal/alerts`, throttle 5 min par `conditionId`) |
...
Plan : [`plans/archived/2026-07-09_PLAN_UI_CRYPTO_ALGO_TUNABLES.md`](../plans/archived/2026-07-09_PLAN_UI_CRYPTO_ALGO_TUNABLES.md).
