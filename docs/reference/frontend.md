...
| **Crypto-Algo** | `CryptoAlgoPage` : onglets **Vue d'ensemble** / **Données** (`CryptoAlgoDataTab`) ; marchés, surveillance, settings (`CryptoAlgoSettingsDialog` : General / Entrée / Sortie / Autotrack), Rapport + Hub Système |
| **Weather Algo** | `WeatherAlgoPage` : onglets Marchés / Positions / Villes / **Données** / **Backtest** / **Stratégies** / Paramètres — voir arbre §3 et [`weather-algo.md`](./weather-algo.md) |
| **Système** | `SystemPage` : Overview (`SystemOverviewPage`), Rapports, Snapshots (**sim** / **réel** via `SnapshotsPage` + `SnapshotsPanel`), E2E, Metrics, **Crypto Algo Monitor** (`CryptoAlgoMonitorPage`) |
...
`simulation_balance`, `simulation_snapshot_created`, `real_snapshot_created`,
`real_period_rotated`, `system:audit:started` / `:log` / `:finished`.
...
│   ├── SimulationSnapshotsPanel.tsx  snapshots sim (page Système → Snapshots, mode sim) — unifié avec réel via `SnapshotsPanel` (`mode: 'sim' \| 'real'`)
