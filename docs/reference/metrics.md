...
|-----|------|---------------|--------|-----------------|
| `polywatch_circuit_breaker_open` | Gauge | `name` | **Actif** | Copy-trading (`move-detector.ts`) → `POST /api/internal/metrics/circuit-breaker` → `recordCircuitBreakerState()` côté backend. Label réel : `PolymarketDataAPI`. |
...
| `polywatch_snapshot_created_total` | Counter | `source`, `mode` | **Actif** | `simulation.ts`, `auto-snapshot-loop.ts` — labels `auto`, `manual`, `reset` × `sim`/`real` |
| `polywatch_snapshot_count` | Gauge | `mode` | **Actif** | Synchronisé après create, prune et delete-all (`simulation.ts`, `auto-snapshot-loop.ts`) |
| `polywatch_snapshot_purge_total` | Counter | `mode` | **Actif** | `auto-snapshot-loop.ts` |
...
| `circuit_breaker_open` | Copy-trading `move-detector.ts` → `POST /api/internal/metrics/circuit-breaker` |
...
Voir [`plans/applied/2026-07-05_PLAN_P0_METRIQUES.md`](../plans/applied/2026-07-05_PLAN_P0_METRIQUES.md) :
