...
| `WeatherBucketTick` | `weather_bucket_ticks` | Prix YES/NO d’un bucket actif ; FK `snapshot_id` **ON DELETE CASCADE**. Colonnes dénormalisées `city`, `city_normalized`, `target_date_iso`, `metric` (backfillées depuis le snapshot parent) + `fidelity_minutes` (dérivé de la cadence de snapshot `weatherAlgoPollMs`) ; index `(city_normalized, target_date_iso, recorded_at)`. La timeline peut être **filtrée par intervalle** (`fidelityMinutes`) et la suppression ciblée ville × intervalle est possible via `deleteBucketTickCityInterval(city, fidelityMinutes)`. **Limite** : `weather_evaluation_log` / `weather_market_snapshots` ne portent pas `fidelity_minutes` → le filtre backtest ne s'applique qu'en mode `reevaluate` (warning `replay_fidelity_filter_unsupported` en replay) |
...
| `BacktestExcludedTick` | `backtest_excluded_ticks` | Ticks exclus d'un run backtest (hors plage, ville/métrique manquante, gap) ; FK `run_id` |
...
Journal append-only des mises à jour de config (migration `0047`, puis colonne
`config_kind` ajoutée par la migration `0087` post-split). Champs : `source`
(`api` | `report_apply` | `system`), `config_kind` (`global` | `copy` | `crypto`
| `weather`), `patch_json`, `config_json` (présentation API), `config_fingerprint`
(hash tunables `crypto_algo_*`), `created_at`. Créé à chaque `PUT /api/config/{kind}`
(géré par le routeur `config-per-kind`).
...
- `engine_version` (semver moteur, ex. `0.8.0` via `BACKTEST_ENGINE_VERSION`)
...
**`backtest_excluded_ticks`** — ticks exclus du replay (migration
`AddBacktestExcludedTicks1700000000115`). Champs : `run_id` (FK), `t`
(timestamp exclu), `reason` (motif d'exclusion), `city`, `metric`,
`condition_id`. Index sur `run_id`. Sert à expliquer les `fidelity_warnings`
de type plage manquante / ville absente.
...
| `weather-order-signals` | liste | weather-algo -> `Executor` (worker) |
| `close-signals` | liste | `StrategyProcessing` / weather-algo / closes manuels -> `Executor` B |
...
| `config-changed` | pub/sub | Backend -> worker / copy-trading / crypto-algo / weather-algo |
| `algo-selections-changed` | pub/sub | crypto-algo → services intéressés (notification de changement de sélection de marchés algo ; canal `ALGO_SELECTIONS_CHANGED_CHANNEL`) |
| `backend-ready` | pub/sub + cle TTL | Backend -> services (signal boot pret, cle `backend-ready` EX 60 s) |
| `simulation-reset` | pub/sub | Backend -> worker / copy-trading / crypto-algo / weather-algo (hygiène Redis au reset sim) |
| `heartbeat` | pub/sub | worker + copy-trading + crypto-algo + weather-algo (battement 30 s) |
...
| `weather-algo:heartbeat` | cle SET EX 60 | liveness weather-algo |
