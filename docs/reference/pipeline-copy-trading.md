...
> Voir aussi [`../code/05a-copy-trading.md`](../code/05a-copy-trading.md) et
...
### Filtre `minTimeToClose`

`simMinTimeToClose` / `realMinTimeToClose` (défaut `0` = désactivé). Dans
`copy-entry-pipeline.ts`, une entrée est refusée si le temps restant avant
`endDate` est inférieur au seuil (secondes). Évite d'ouvrir trop près de la
fenêtre pre-close / résolution.
...
- **Retry des sorties forcées** : si une exécution `SELL` de sortie stratégique
  (`SL`, `TRAILING`, `PRE_CLOSE_LOSS`, `PRE_CLOSE_WIN`, `KILL_SWITCH`) échoue avec une erreur
...
5. **Évaluation des sorties** (throttle 50 ms) :
   ...
   - `evaluatePreCloseExit` (`packages/core/src/risk/exit-decision.ts`) — fenêtre
     unique `preCloseSeconds` avant `endDate` (ou tant que `acceptingOrders`
     reste `true` après `endDate`) :
     - si `keepEnabled` (`simPreCloseKeepEnabled` / `realPreCloseKeepEnabled`)
       et `markBid ≥ keepBidThreshold` : **aucune sortie** — la position reste
       ouverte jusqu'à la résolution ;
     - sinon : `PRE_CLOSE_LOSS` si `trigger < 0` **OU** `closure < 0` ;
       **`PRE_CLOSE_WIN`** si la position n'est pas perdante.
     Défaut copy : keep **désactivé** (`simPreCloseKeepEnabled` /
     `realPreCloseKeepEnabled` = `false`) → une gagnante dans la fenêtre sort
     en `PRE_CLOSE_WIN`. L'exécuteur bloque aussi un `PRE_CLOSE_LOSS` dont le
     fill serait non négatif.
     **Liquidity gate** : `PRE_CLOSE_LOSS` est annulé
     ...
     > La sortie `TIME_EXIT` a été retirée — `evaluatePositionExit` ne gère
     > que SL/TP/trailing + pre-close. SL copy : `slConfirmationTicks`
     > évaluations consécutives avant émission (défaut 2).
