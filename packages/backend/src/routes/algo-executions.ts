...
import { CopiedPosition, Execution, MarketService } from '@polywatch/core';
...
export function createAlgoExecutionsRouter(ds: DataSource): Router {
  const router = Router();
  ...
  router.get('/', requireJwt, async (req, res) => {
    ...
    // Only ALGO_* executions
    qb.andWhere('e.reason LIKE :algoPattern', { algoPattern: 'ALGO_%' });

    // Optional conditionId filter (joins via CopiedPosition).
