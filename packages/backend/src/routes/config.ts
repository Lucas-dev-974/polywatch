...
const cryptoAlgoExitDefaultsMap = z
  .record(
    ...
    z
      .object({
        slPercent: z.number().finite().min(0).max(100).optional(),
        tpPercent: z.number().finite().min(0).max(100).optional(),
        trailingPercent: z.number().finite().min(0).max(100).optional(),
        trailingActivationPercent: z.number().finite().min(0).max(100).optional(),
      })
      .strict(),
  )
  .nullable();
...
export const riskConfigUpdateSchema = z
  .object({
    ...
    maxExposurePusd: nonNegNumber,
    maxDailyLossPusd: nonNegNumber,
    maxPositionSizePusd: nonNegNumber,
    ...
    simTrailingActivationPercent: z.number().finite().min(0).max(100),
    realSlCloseMaxRetries: nonNegInt,
    realTrailingEnabled: z.boolean(),
    realTrailingPercent: z.number().finite().min(0).max(100),
    realTrailingActivationPercent: z.number().finite().min(0).max(100),
    simSlEnabled: z.boolean(),
    simTpEnabled: z.boolean(),
    simSlPercent: z.number().finite().min(0).max(100),
    simTpPercent: z.number().finite().min(0).max(100),
    realSlPercent: z.number().finite().min(0).max(100),
    realTpPercent: z.number().finite().min(0).max(100),
    ...
    cryptoAlgoTrailingPercent: z.number().finite().min(0).max(100).nullable(),
    cryptoAlgoTrailingActivationPercent: z.number().finite().min(0).max(100).nullable(),
    cryptoAlgoSlEnabled: z.boolean(),
    cryptoAlgoTpEnabled: z.boolean(),
    cryptoAlgoTrailingEnabled: z.boolean(),
    cryptoAlgoSlPercent: z.number().min(0).max(100).nullable(),
    cryptoAlgoTpPercent: z.number().min(0).max(100).nullable(),
