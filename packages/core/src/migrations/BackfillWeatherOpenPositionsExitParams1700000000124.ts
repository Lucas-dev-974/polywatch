 ...
 *
 * This migration updates open WEATHER_* positions so they reflect the
 * *current* strategy config for their strategyId + mode. It is safe to run
 * multiple times (idempotent): existing non-NULL values are preserved via
 * COALESCE, and positions whose strategy config still has the leg disabled
 * remain NULL.
 *
 ...
 */
export class BackfillWeatherOpenPositionsExitParams1700000000124 implements MigrationInterface {
  name = 'BackfillWeatherOpenPositionsExitParams1700000000124';
