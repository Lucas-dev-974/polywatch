...
export function useRealSnapshots() {
  ...
  const [items, setItems] = createSignal<RealStateSnapshotSummary[]>([]);
  const [chartItems, setChartItems] = createSignal<RealStateSnapshotSummary[]>([]);
  const [total, setTotal] = createSignal(0);
  const [page, setPage] = createSignal(0);
  const [filters, setFilters] = createSignal<RealSnapshotListFilters>({
    source: 'all',
  });
  const [sessions, setSessions] = createSignal<RealSessionSummary[]>([]);
  ...
  const [sessionFilters, setSessionFilters] =
    createSignal<RealSessionListFilters>({ status: 'all' });
  const [currentSession, setCurrentSession] =
    createSignal<RealSessionSummary | null>(null);
  const [selected, setSelected] = createSignal<Set<number>>(new Set());
  const [selectedItems, setSelectedItems] = createSignal<
    Map<number, RealStateSnapshotSummary>
  >(new Map());
  ...
  const [details, setDetails] = createSignal<Map<number, RealStateSnapshotDetail>>(
    new Map(),
  );
  ...
  const sessionConfigDiffPreview = (
    ...
    return {
      loading: false,
      error: false,
      lines: buildConfigDiffPreviewLines('real', inputs, sessionId, refId),
    };
  };

  function mergeSelectedSummaries(pageItems: RealStateSnapshotSummary[]) {
  ...
  function mergeSelectedSessions(pageItems: RealSessionSummary[]) {
  ...
  async function loadCurrentSession() {
    try {
      setCurrentSession(await fetchCurrentRealSession());
    } catch {
  ...
  async function loadSessions() {
    setLoading(true);
    try {
      const data = await fetchRealSessions(
        SESSION_PAGE_SIZE,
  ...
  async function loadChartSeries() {
    try {
      const data = await fetchRealSnapshots(
  ...
  async function loadDetailsForSelected(ids: number[]) {
    ...
    const fetched = await Promise.all(
      missing.map(async (id) => {
        const detail = await fetchRealSnapshotDetail(id);
  ...
  function syncReferenceAfterSelection(
    next: Set<number>,
    summaries: RealStateSnapshotSummary[],
  ) {
  ...
  function setSourceFilter(source: RealSnapshotListFilters['source']) {
  ...
  function setSessionStatusFilter(
    status: RealSessionListFilters['status'],
  ) {
  ...
  async function renameSession(id: number, label: string | null) {
    const updated = await updateRealSession(id, { label });
  ...
  async function removeSession(id: number, deleteSnapshots: boolean) {
    const result = await deleteRealSession(id, deleteSnapshots);
  ...
  async function deleteSelectedSessions() {
    ...
    const deleteSnapshots =
      totalSnapshots > 0
        ? confirm(
            `${ids.length} période(s) — ${totalSnapshots} snapshot(s).\n\nOK = supprimer périodes + snapshots\nAnnuler = supprimer seulement les périodes`,
          )
        : false;
    if (
      !confirm(
        deleteSnapshots
          ? `Confirmer la suppression de ${ids.length} période(s) et de leurs snapshots ?`
          : `Supprimer ${ids.length} période(s) ?`,
      )
    ) {
      return;
    }
    setDeleting(true);
    try {
      await Promise.all(ids.map((id) => deleteRealSession(id, deleteSnapshots)));
      clearSessionSelection();
      await refresh();
    } finally {
      setDeleting(false);
  ...
  async function deleteAll() {
    ...
    const confirmed = confirm(
      view() === 'snapshots'
        ...
        : `Supprimer tous les snapshots ?\n\nLes périodes fermées resteront (sans snapshots).`,
    );
    if (!confirmed) return false;
    setDeleting(true);
    try {
      await deleteAllRealSnapshots();
      clearSelection();
      setDetails(new Map());
      setPage(0);
      await refresh();
  ...
  onMount(() => {
    ...
    socket.on('real_period_rotated', onRefresh);
    socket.on('real_snapshot_created', onRefresh);
    onCleanup(() => {
      socket.off('real_period_rotated', onRefresh);
      socket.off('real_snapshot_created', onRefresh);
