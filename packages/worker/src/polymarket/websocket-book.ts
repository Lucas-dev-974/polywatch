...
export class PolymarketBookWebSocket {
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const connectTimeout = setTimeout(() => {
        if (settled) return;
        settled = true;
        log.warn({ wsUrl: config.wsUrl }, 'polymarket ws connect timeout');
        this.healthy = false;
        this.scheduleReconnect();
        reject(new Error('polymarket ws connect timeout'));
      }, WS_CONNECT_TIMEOUT_MS);
      ...
      try {
        log.info({ wsUrl: config.wsUrl }, 'connecting to polymarket ws');
        ...
        this.ws.on('close', (code: number, reason: Buffer) => {
          ...
          this.scheduleReconnect();
          onSettled(() => {
            reject(new Error(`polymarket ws closed before open (code=${code})`));
          });
        });
  ...
  async syncAll(staleThresholdMs = 30_000): Promise<void> {
    if (this.healthy) {
      const now = Date.now();
      ...
      for (const assetId of assets) {
        ...
        const age = book?.updatedAt ? now - book.updatedAt.getTime() : Infinity;
      ...
      if (synced === 0) {
        log.debug('syncAll skipped — all books fresh and WS healthy');
      }
      return;
    }
